...
Decouples training from live LLM calls: run this once against a real backend,
commit the resulting `configs/objects/<name>.json` files, and every later
training run -- including on a SLURM node with no internet -- reads the
snapshot instead of calling an LLM again.
...
Usage (from the repo root):
    ...
    PYTHONPATH=src python src/scripts/extract_object_params.py --backend groq --model openai/gpt-oss-120b
"""
