...
``shape``/``size``/``density``/``friction`` map directly onto a robosuite primitive
object. ``grip_force_min_N``/``max_N``/``crush_force_N`` drive `rl/env.py`'s reward
shaping against a real MuJoCo fingertip force sensor (`objects/force_gripper.py`);
``mass_class``/``fragile`` are descriptive metadata only. All fields are clamped in
`__post_init__` to ranges that stay graspable by a Panda gripper and numerically
stable in MuJoCo, since an LLM can return wild values (a "boulder" at 50 kg).
"""
...
#: Per-shape, per-component (lo, hi) size bounds in metres, chosen so every dimension
#: stays within a Panda parallel-jaw gripper's graspable range.
...
#: kg/m^3, spanning thin-walled glass through solid steel.
_DENSITY_RANGE = (50.0, 9000.0)
#: (sliding, torsional, rolling) coefficient-of-friction bounds.
_FRICTION_BOUNDS: tuple[tuple[float, float], tuple[float, float], tuple[float, float]] = (
    (0.05, 1.5),
    (0.0, 0.05),
    (0.0, 0.01),
)
#: Newtons, per finger -- what a Panda gripper can realistically exert.
_GRIP_FORCE_RANGE = (0.1, 200.0)
#: N/m stiffness range, soft/compliant to rigid/brick-like.
_SPRING_RANGE = (20.0, 10000.0)
#: Newtons, per finger -- the contact force that damages the object.
...
class ObjectParams:
    ...
    def __post_init__(self) -> None:
        # JSON round-trips tuples back as lists; normalise back to tuples.
    ...
    def reaction_force_N(self, aperture_mm: float) -> float:
        """Spring-compression contact force (N) at a given gripper aperture. Not currently used by `rl/env.py`."""
...
#: one-hot position always means the same shape.
_Z_SHAPES: tuple[Shape, ...] = tuple(_SIZE_DIMS)  # ("box", "cylinder", "ball")
_Z_MAX_SIZE_DIMS = max(_SIZE_DIMS.values())

#: one-hot shape + size (zero-padded) + density + friction(3). `mass_kg` is excluded
#: since it's a deterministic function of density and size already in z.
...
def sample_object_params(shapes: Sequence[Shape] = _Z_SHAPES, name: str = "domain_random") -> ObjectParams:
    ...
    Used by the paradigm-switch baselines for domain randomization (see
    `objects/lift_object_task.ParamLift`). Draws from the global `numpy.random` API to
    stay in sync with robosuite's own placement sampler under a single seed.
    """
...
def object_params_to_z(params: ObjectParams) -> np.ndarray:
    ...
    Layout: one-hot shape | size (zero-padded) | density | friction (3).
    """
...
#: z-vector index the noise starts from -- the one-hot shape block stays exact.
...
def object_params_to_noisy_z(params: ObjectParams, rel_noise_std: float = 0.1) -> np.ndarray:
    ...
    Models a noisy sysID-style estimate of the object, for the `pi_param` paradigm-
    switch variant. `rel_noise_std=0.0` returns the exact (noiseless) z.
    """
